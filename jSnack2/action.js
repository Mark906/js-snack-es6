$(document).ready(function(){
    ...
    do{ //Faccio inserire l'estremo di sinistra.
        a = parseInt(prompt('Inserisci un\'intervallo. Digita il primo valore (piu piccolo): '));
    }while (a > l || a < 1);

    do{ //Faccio inserire l'estremo di destra.
        b = parseInt(prompt('Digita il secondo valore(piu grande): '));
    } while (b < a || b > l);
    ...
    var arrayFilter = nomi.filter((nome, index) => {
        return (a-1) <= index && (b-1) >= index;
    });

    //var arrayFilter = nomi.filter((nome, index) => (a-1) <= index && (b-1) >= index);

    console.log('Con l\'utilizzo di filter: ' + arrayFilter);
})
